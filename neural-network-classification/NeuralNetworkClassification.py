...
def make_svc_params_grid(my_args):
    ...
    model_params = {
        "model__C": [ 0.1, 1.0 ],
        "model__kernel": [ "linear", "rbf", "sigmoid", "poly" ], # "poly", 
        "model__degree": [ 1, 2, 3, 4, 5, 6 ], # 1
...
def make_numerical_predictor_params_big(my_args):
    ...
    if my_args.numerical_missing_strategy:
        params["features__numerical__missing-data__strategy"] = ['mean']
    if my_args.use_polynomial_features:
        params["features__numerical__polynomial-features__degree"] = [ 2, 3 ]
    return params
...
def make_decision_tree_params_grid_big(my_args):
    ...
    tree_params = {
        ...
        "model__max_depth": [ 4, 5, None ],
        "model__min_samples_split": [ 2, 3, 4, 8, 12 ],
        "model__min_samples_leaf":  [ 2, 3, 4, 8, 12 ],
        "model__max_features":  [ "sqrt" ],
        "model__max_leaf_nodes": [ None ],
        "model__min_impurity_decrease": [ 0.0, 0.01, 0.02, 0.04],
    }
    params.update(tree_params)
    return params
...
def make_svc_params_grid_big(my_args):
    params = make_predictor_params_big(my_args)
    model_params = {
        "model__C": [ 0.1, 0.5, 0.8, 1.0, 2.0],
        "model__kernel": [ "poly"], 
        "model__degree": [ 1, 2, 3],
        "model__gamma": [ 0.01, 0.05, 0.1, 0.3, 0.5, 0.8, 1.0],
...
def do_grid_search(my_args):
    ...
    search_grid = sklearn.model_selection.GridSearchCV(pipeline, fit_params, scoring="f1_micro", n_jobs=-1, verbose=1)
    search_grid.fit(X, y)
...
def do_random_search(my_args):
    ...
    search_grid = sklearn.model_selection.RandomizedSearchCV(pipeline, fit_params, scoring="f1_micro", n_jobs=-1, verbose=1, n_iter=my_args.n_search_iterations)
    search_grid.fit(X, y)
...
def do_svc_random_search(my_args):
    ...
    search_grid = sklearn.model_selection.RandomizedSearchCV(pipeline, fit_params, scoring="f1_micro", n_jobs=-1, verbose=1, # 10
...
def parse_args(argv):
    ...
    parser.add_argument('action', default='DT',
                        choices=[ "DT", "score", "show-model", "cross-validate", "cross-validate-svc", "grid-search", "show-best-params", "random-search",
                                  "SVC", "SVC-grid-search", "SVC-random-search","ANN"], 
